...
if [ "${BUILD_KSU}" -eq 1 ]; then
    make "${MAKE_ARGS[@]}" "${DEFCONFIG}" vendor/common.config vendor/debugfs.config vendor/X00TD.config vendor/extra/ksu.config 2>&1 | tee -a "${BUILD_LOG}" || error "Defconfig step failed"
else
    make "${MAKE_ARGS[@]}" "${DEFCONFIG}" vendor/common.config vendor/debugfs.config vendor/X00TD.config 2>&1 | tee -a "${BUILD_LOG}" || error "Defconfig step failed"
fi
